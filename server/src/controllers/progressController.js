...
exports.logProgress = async (req, res) => {
  try {
    ...
    let progress;
    if (existing) {
      progress = await Progress.findByIdAndUpdate(existing._id, progressData, { new: true });
    } else {
      progress = await Progress.create(progressData);
    }

    successResponse(res, progress, existing ? 'Progress updated.' : 'Progress logged.', 201);
  } catch (error) {
...
exports.getProgressHistory = async (req, res) => {
  try {
    ...
    paginatedResponse(res, logs, { page, limit, total, totalPages: Math.ceil(total / limit) }, 'Progress history fetched.');
  } catch (error) {
    errorResponse(res, error.message, 500);
  }
};
...
exports.addTrainerNotes = async (req, res) => {
  try {
    ...
    const progress = await Progress.findById(req.params.progressId).populate('member');

    if (!progress) return errorResponse(res, 'Progress log not found.', 404);

    // Verify this member belongs to trainer
    const member = await User.findOne({ _id: progress.member, trainer: req.user._id });
...
exports.getProgressAnalytics = async (req, res) => {
  try {
    const memberId = req.params.memberId || req.user._id;
    const logs = await Progress.find({ member: memberId }).sort({ date: 1 }).limit(90);

    if (!logs.length) return successResponse(res, {}, 'No progress data found.');
    ...
    const analytics = {
      ...
      workoutCompletionRate: logs.length ? ((workoutsDone.length / logs.length) * 100).toFixed(1) : 0,
      weightHistory: withWeight.map(l => ({ date: l.date, weight: l.weight })),
      avgCaloriesConsumed: withCalories.length
        ? (withCalories.reduce((s, l) => s + l.caloriesConsumed, 0) / withCalories.length).toFixed(0)
        : null,
      avgEnergyLevel: logs.filter(l => l.energyLevel).length
        ? (logs.filter(l => l.energyLevel).reduce((s, l) => s + l.energyLevel, 0) / logs.filter(l => l.energyLevel).length).toFixed(1)
        : null,
      avgSleepHours: logs.filter(l => l.sleepHours).length
        ? (logs.filter(l => l.sleepHours).reduce((s, l) => s + l.sleepHours, 0) / logs.filter(l => l.sleepHours).length).toFixed(1)
        : null,
